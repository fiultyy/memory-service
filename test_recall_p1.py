...
# 1. 默认 list[dict] 每条带 _snaptag + 建 mem-*.md
...
assert tag["mem_path"] == f"memory/mem-{fid}.md"

mem_file = mem_dir / f"mem-{fid}.md"
assert mem_file.exists(), "mem-*.md 应被创建"
...
assert not (mem_dir / f"mem-{fid}.md.tmp").exists(), "原子写不应留 .tmp"
...
assert mem_file.exists() and not (mem_dir / f"mem-{fid}.md.tmp").exists()

print("✓ P1 recall: 建 mem-*.md(瘦/原子)+ _snaptag + 默认/nested/verbose shape + 不碰 MEMORY + 幂等")
shutil.rmtree(tmp)
